...
export default class PublicacionesController {
    async setRegistroPublicacion({request, response}: HttpContextContract) {
        try{
            const dataPublicaciones = request.only([
                'codigo_publicacion', 'codigo_usuario', 'tituto', 'cuerpo'
            ])
            const codigoPublicacion = dataPublicaciones.codigo_publicacion;
            const codigoPublicacionExistente: Number = await this.getValidarPublicacioneExistente(codigoPublicacion)
            if (codigoPublicacionExistente === 0) {
                await Publicaciones.create(dataPublicaciones)
                response.status(200).json({"msg":"Registro de publicacion compeltado con exito"})
            } else {
    ...
    private async getValidarPublicacioneExistente(codigo_publicacion: Number) : Promise <Number> {
